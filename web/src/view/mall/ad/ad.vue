...
<template>
  <div>
      ...
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">
          <el-form-item label="类型">
              <el-select v-model="searchInfo.type" placeholder="请选择" style="width:100%" clearable>
                <el-option v-for="(item,key) in adTypeOptions" :key="key" :label="item.label" :value="item.value" />
          </el-select>
                  </el-form-item>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="广告名" prop="name" width="120" />
        <el-table-column align="left" label="类型" prop="type" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.type,adTypeOptions) }}
            </template>
        </el-table-column>
        <el-table-column align="left" label="图片"  width="120" >
           <template #default="scope">
            <CustomPic pic-type="file" :pic-src="scope.row.image" />
          </template>
        </el-table-column>
        <el-table-column align="left" label="图片宽度" prop="width" width="120" />
        <el-table-column align="left" label="图片高度" prop="height" width="120" />
        <el-table-column align="left" label="介绍" prop="introduce" width="120" />
        <el-table-column align="left" label="链接地址" prop="url" width="200" />
        <el-table-column align="left" label="是否激活" prop="status" width="120">
                <template #default="scope">
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" label-width="80px">
        ...
        <el-form-item label="过期时间:">
          <el-date-picker v-model="formData.expirationTimeDate" type="date" style="width:100%" placeholder="选择日期" clearable />
        </el-form-item>
      </el-form>
      <template #footer>
...
const setOptions = async () =>{
    adTypeOptions.value = await getDictFunc('adType')
}
...
const updateAdFunc = async(row) => {
    ...
    if (res.code === 0) {
        ...
        formData.value.expirationTimeDate= new Date(formData.value.expirationTime)
...
const enterDialog = async () => {
      let res
      formData.value.expirationTime= formData.value.expirationTimeDate.getTime()
      switch (type.value) {
        case 'create':
