<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="退货原因" prop="reason" width="120" />
        <el-table-column align="left" label="排序标记" prop="sort" width="120" />
        <el-table-column align="left" label="是否已退货" prop="status" width="120">
        ...
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="text" icon="edit" size="small" class="table-button" @click="updateOrderReturnFunc(scope.row)">变更</el-button>
...
// 全量引入格式化工具 请按需保留
import { getDictFunc, formatDate,formatUnixDate, formatBoolean, filterDict } from '@/utils/format'
...
const updateOrderReturnFunc = async(row) => {
    const res = await findOrderReturn({ orderId: row.orderId })
    type.value = 'update'
    if (res.code === 0) {
