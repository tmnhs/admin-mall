<template>
  <div>
    <div class="gva-search-box">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">
         <el-form-item label="编号">
          <el-input v-model="searchInfo.ID" placeholder="请输入分类编号" />
        </el-form-item>
        <el-form-item label="分类名">
          <el-input v-model="searchInfo.name" placeholder="搜索条件" />
        </el-form-item>
           <el-form-item label="分类级别">
          <el-select v-model="searchInfo.level" placeholder="请选择" style="width:100%" clearable>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="上级分类id" prop="parentId" width="120" />
        <el-table-column align="left" label="分类级别" prop="level" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.level,categoryLevelOptions) }}
            </template>
        </el-table-column>
        <el-table-column align="left" label="物品总数" prop="itemCount" width="120" />
        <el-table-column align="left" label="显示状态" prop="showStatus" width="120">
             <template #default="scope">
                      <el-switch v-model="scope.row.showStatus" active-color="#13ce66" inactive-color="#ff4949"  clearable disabled></el-switch>
           </template>
        </el-table-column>
        <el-table-column align="left" label="排序权值" prop="sort" width="120" />
        <el-table-column align="left" label="图标"  width="120" >
           <template #default="scope">
            <CustomPic pic-type="file" :pic-src="scope.row.icon" />
          </template>
        </el-table-column>
        <el-table-column align="left" label="分类描述" prop="description" width="120" />
        <el-table-column align="left" label="按钮组" width="120">
            <template #default="scope">
            <el-button type="text" icon="edit" size="small" class="table-button" @click="updateCategoryFunc(scope.row)">变更</el-button>
...
const onSubmit = () => {
  page.value = 1
  pageSize.value = 10
  if (searchInfo.value.showStatus === ""){
      searchInfo.value.showStatus=null
  }
  getTableData()
}
