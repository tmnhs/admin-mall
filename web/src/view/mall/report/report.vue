  ...
  <div>
    <div class="gva-search-box">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">
        <el-form-item label="举报物品id">
          <el-input v-model="searchInfo.itemId" placeholder="搜索条件" />
        </el-form-item>
               <el-form-item label="类型">
          <el-select v-model="searchInfo.type" placeholder="请选择" style="width:100%" clearable>
            <el-option v-for="(item,key) in reportTypeOptions" :key="key" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="举报物品id" prop="itemId" width="120" />
        <el-table-column align="left" label="类型" prop="type" width="180">
            <template #default="scope">
            {{ filterDict(scope.row.type,reportTypeOptions) }}
            </template>
        </el-table-column>
        <el-table-column align="left" label="是否审核通过" prop="status" width="120">
                  <template #default="scope">
                      <el-switch v-model="scope.row.status" active-color="#13ce66" inactive-color="#ff4949"  clearable disabled></el-switch>
                  </template>
        </el-table-column>
          <el-table-column align="left" label="举报时间" prop="createdTime" width="200" >
            <template #default="scope">{{ formatUnixDate(scope.row.createdTime) }}</template>
        </el-table-column>
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="text" icon="edit" size="small" class="table-button" @click="updateReportFunc(scope.row)">变更</el-button>
...
const setOptions = async () =>{
    reportTypeOptions.value = await getDictFunc('reportType')
}
...
const updateReportFunc = async(row) => {
    ...
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rereport
