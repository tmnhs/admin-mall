<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="是否已购得" prop="status" width="120">
              <template #default="scope">
        ...
        <el-table-column align="left" label="按钮组" width="120">
            <template #default="scope">
            <el-button type="text" icon="edit" size="small" class="table-button" @click="updatePurchaseItemFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getPurchaseItemList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deletePurchaseItemByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updatePurchaseItemFunc = async(row) => {
    const res = await findPurchaseItem({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deletePurchaseItemFunc = async (row) => {
    const res = await deletePurchaseItem({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
